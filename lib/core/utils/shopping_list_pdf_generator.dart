...
class ShoppingListPdfGenerator {
  ...
  static Future<File?> generateShoppingListPdf(List<Meal> meals) async {
    ...
    // Aggregate groceries.
    // Suggested meals have full Grocery objects (name + amount).
    // History meals only have groceryNames (strings, no amounts).
    // Use Map<String, int?> where null means amount is unknown.
    ...
    for (var meal in meals) {
      if (meal.groceries != null && meal.groceries!.isNotEmpty) {
        ...
        for (var name in meal.groceryNames!) {
