...
class MealDataSource {
  ...
  /// Fetches full meal details (with groceries + amounts) for a batch of history IDs.
  Future<List<Meal>> getMealHistoryDetails(List<int> ids) async {
    try {
      final response = await _dio.post(
        ApiConstants.mealHistoryDetails,
        data: ids,
      );
      ...
      if (response.data is Map<String, dynamic>) {
        ...
        if (data is List) {
          return data.map((e) {
            ...
            // API returns "mealHistoryId"; map it to "id" for the Meal model
            map['id'] = map['mealHistoryId'];
